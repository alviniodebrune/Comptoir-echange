...
export default function Page() {
  ...
  const [form, setForm] = useState({ link: "", type: "sub", ppa: 2, count: 5 });
  ...
  const publish = async () => {
    ...
    const ppa = Number(form.ppa);
    const count = Number(form.count);
    if (!ppa || ppa < 1 || !count || count < 1) {
      setFormError("Indique un nombre de points et d'actions valides.");
      return;
    }
    setPosting(true);
    ...
    setForm({ link: "", type: "sub", ppa: 2, count: 5 });
          ...
          <div className="bg-white border border-line rounded p-5">
            ...
            <div className="flex gap-3 mb-3">
              <div className="flex-1">
                <label className="text-xs text-inksoft font-semibold">POINTS / ACTION</label>
                <input
                  type="number"
                  min={1}
                  value={form.ppa}
                  onChange={(e) => setForm({ ...form, ppa: e.target.value })}
                  className="w-full px-3 py-2.5 border border-line rounded mt-1.5 font-mono"
                />
              </div>
              <div className="flex-1">
                <label className="text-xs text-inksoft font-semibold">NB D'ACTIONS</label>
                <input
                  type="number"
                  min={1}
                  value={form.count}
            ...
            <p className="text-sm text-inksoft mb-3">
              ...
              <strong className="font-mono text-ink">
                {(Number(form.ppa) || 0) * (Number(form.count) || 0)} points
              </strong>{" "}
